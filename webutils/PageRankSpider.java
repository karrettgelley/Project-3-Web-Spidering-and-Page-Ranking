...
public class PageRankSpider extends Spider {
    ...
    /**
     * The number of pages indexed. In the default implementation a page is
     * considered to be indexed only if it is written to a file.
     */
     ...
     * </ul>
     * <p/>
     * Each option has a corresponding <code>handleXXXCommandLineOption</code>
     * function that will be called when the option is found. Subclasses may find it
     * convenient to change how options are handled by overriding those methods
     * instead of this one. Only the above options will be dealt with by this
     * function, and the input array will remain unchanged. Note that if the flag
     * for an option appears in the input array, any value associated with that
     * option will be assumed to follow. Thus if a "-c" flag appears in
     * <code>args</code>, the next value in <code>args</code> will be blindly
     * treated as the count.
     *
     * @param args Array of arguments as passed in from the command line.
     */
     ...
     * <p>
     * This implementation sets <code>retriever</code> to a
     * {@link SafeHTMLPageRetriever SafeHTMLPageRetriever}.
     */
     ...
     * <p>
     * This implementation sets <code>saveDir</code> to <code>value</code>.
     *
     ...
     * <p>
     * This implementation sets <code>maxCount</code> to the integer represented by
     * <code>value</code>.
     *
     ...
     * <p>
     * This implementation adds <code>value</code> to the list of links to visit.
     *
     ...
     * <p>
     * This implementation sets a flag that will be used in <code>go</code> to pause
     * briefly before downloading each page.
     */
    ...
    /**
     * Performs the crawl. Should be called after <code>processArgs</code> has been
     * called. Assumes that starting url has been set.
     * <p>
     * This implementation iterates through a list of links to visit. For each link
     * a check is performed using {@link #visited visited} to make sure the link has
     * not already been visited. If it has not, the link is added to
     * <code>visited</code>, and the page is retrieved. If access to the page has
     * been disallowed by a robots.txt file or a robots META tag, or if there is
     * some other problem retrieving the page, then the page is skipped. If the page
     * is downloaded successfully {@link #indexPage indexPage} and
     * {@link #getNewLinks getNewLinks} are called if allowed. <code>go</code>
     * terminates when there are no more links to visit or
     * <code>count &gt;= maxCount</code>
     */
    public void doCrawl() {
    ...
    protected boolean linkToHTMLPage(Link link) {
        ...
        if (extension.equals("") || extension.equalsIgnoreCase("html") || extension.equalsIgnoreCase("htm")
