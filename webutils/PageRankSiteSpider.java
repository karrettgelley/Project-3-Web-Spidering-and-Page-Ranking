...
public class PageRankSiteSpider extends PageRankSpider {
     ...
     *
     * @return A list of links on <code>page</code> that have the same host as
     *         <code>url</code>.
     */
    public List<Link> getNewLinks(HTMLPage page) {
        ...
        Node currentPageNode = crawlGraph.getNode(url.toString());
        ...
        while (iterator.hasNext()) {
            ...
            if (!url.getHost().equals(link.getURL().getHost())) {
                ...
                // Add an edge to every Node associated with a link
                // of the current page Node
                Node outNode = crawlGraph.getNode(link.getURL().toString());
